@testset "van der Pol oscillator test" begin
    @testset "Van der Pol basics" begin
        ...
        savefig(plt1, joinpath(PATHS[:vanderpol], "00_vanderpol_time.pdf"))
        savefig(plt2, joinpath(PATHS[:vanderpol], "01_vanderpol_phase_space.pdf"))
        savefig(plt3, joinpath(PATHS[:vanderpol], "02_vanderpol_poincare_section.pdf"))
        savefig(plt4, joinpath(PATHS[:vanderpol], "03_vanderpol_return_maps.pdf"))
        savefig(plt5, joinpath(PATHS[:vanderpol], "04_vanderpol_amplitudes.pdf"))
        savefig(plt6, joinpath(PATHS[:vanderpol], "05_vanderpol_amplitudes_loglog.pdf"))
        @test true
    end
    @testset "van der pol 引き込み現象" begin
        # 引き込み現象
        ...
        for ϵ in ϵ_list
            ...
            savefig(plt1, joinpath(PATHS[:vanderpol], "10_vanderpol_phase_space_ϵ$(ϵ).pdf"))
            savefig(plt2, joinpath(PATHS[:vanderpol], "11_vanderpol_time_position_ϵ$(ϵ).pdf"))
            savefig(plt3, joinpath(PATHS[:vanderpol], "12_vanderpol_fft_ϵ$(ϵ).pdf"))
        end
        @test true
    end
    @testset "van der pol Arnold Tongue" begin
        # 引き込み現象
        ϵ_list = [0.0, 1.0, 2.0, 4.0]
        df = 0.04
        dω = 0.04
